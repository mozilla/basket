...
from basket.news.backends.exacttarget import ExactTarget, ExactTargetDataExt
from basket.news.backends.exacttarget_rest import ETRestError, ExactTargetRest
from basket.news.celery import app as celery_app
from basket.news.models import FailedTask, Newsletter, Interest
from basket.news.newsletters import get_sms_messages, is_supported_newsletter_language
from basket.news.utils import (generate_token, get_user_data, MSG_USER_NOT_FOUND,
...
def et_task(func):
    ...
    @celery_app.task(base=ETTask, bind=True, name='news.tasks.{}'.format(func.__name__))
    @wraps(func)
    def wrapped(self, *args, **kwargs):
        start_time = kwargs.pop('_start_time', None)
        if start_time:
            total_time = int((time() - start_time) * 1000)
            statsd.timing(self.name, total_time)
...
@celery_app.task(name='news.tasks.snitch')
